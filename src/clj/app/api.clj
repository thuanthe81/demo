...
(def route-handler-specs
  {::route/users              {:get  {:summary    "Get all users in the database. Return their details."
                                      :middleware [[mdw/authenticated?]
                                                   [mdw/grant-access-as-role :admin]]
                                      :parameters {:header {:authorization schema/jwt-bearer?}}
                                      :handler    user-handler/get-users-handler}
   ...
   ::route/user-role          {:put {:summary    "Update user role"
                                     ...
                                     :parameters {:header {:authorization schema/jwt-bearer?}
                                                  ...
                                                  :body   {:role keyword?}}
                                     :handler    user-handler/update-user-handler}}
   ::route/user-refresh-token {:get {:summary    "Refresh access token for user"
                                     :middleware [[mdw/authenticated?]]
                                     :parameters {:header {:authorization schema/jwt-bearer?}}
                                     :handler    user-handler/refresh-token-handler}}
